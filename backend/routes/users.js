...
router.post('/disponibilidade', authMiddleware, async (req, res) => {
  try {
    ...
    console.log('Data input parseada:', { ano: anoInput, mes: mesInput, dia: diaInput });
    ...
    const conflito = user.disponibilidade.some(d => {
      ...
      console.log(`Comparando com disponibilidade existente:`, {
        ...
        anoExist, mesExist, diaExist,
        horarioExist: `${d.horario_inicio} - ${d.horario_fim}`
      });
      
      // Verifica se é o mesmo dia
      const mesmoDia = anoExist === anoInput && mesExist === mesInput && diaExist === diaInput;
      ...
      const temSobreposicao = inicioNovoMin < fimExistMin && fimNovoMin > inicioExistMin;
...
router.get('/tutor/:id/disponibilidade', authMiddleware, async (req, res) => {
  try {
    ...
    const disponibilidadesComStatus = disponibilidadesFuturas.map(d => {
      ...
      const [horaInicioDisp, minInicioDisp] = d.horario_inicio.split(':').map(Number);
      const [horaFimDisp, minFimDisp] = d.horario_fim.split(':').map(Number);
      
      // Criar datetime completo do slot de disponibilidade
      const inicioSlot = new Date(anoDisp, mesDisp, diaDisp, horaInicioDisp, minInicioDisp, 0, 0);
      const fimSlot = new Date(anoDisp, mesDisp, diaDisp, horaFimDisp, minFimDisp, 0, 0);
      ...
      const jaAgendado = sessoesAgendadas.some(s => {
        ...
        const mesmoDia = inicioSessao.getFullYear() === anoDisp &&
  ...
  try {
    ...
    if (sessaoExistente) {
      return res.status(400).json({ 
        error: 'Não é possível remover esta disponibilidade pois há uma sessão agendada neste horário' 
      });
    }
