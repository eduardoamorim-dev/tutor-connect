...
const verificarConflito = async (tutorId, dataInicio, dataFim, sessaoIdExcluir = null) => {
  const query = {
    tutor: tutorId,
...
router.post('/create', authMiddleware, async (req, res) => {
  try {
    const { tutorId, disciplina, data_hora_inicio, data_hora_fim, observacoes } = req.body;
    ...
    if (tutorId === req.userId) {
      return res.status(400).json({ error: 'Você não pode agendar uma sessão consigo mesmo' });
    }
    ...
    if (dataInicio >= dataFim) {
      return res.status(400).json({ error: 'A hora de término deve ser depois da hora de início' });
    }
    
    if (dataInicio < new Date()) {
      return res.status(400).json({ error: 'Não é possível agendar sessões no passado' });
    }
    
    // Verificar conflito de horário
    const temConflito = await verificarConflito(tutorId, dataInicio, dataFim);
    if (temConflito) {
      return res.status(400).json({ error: 'O tutor já tem uma sessão agendada neste horário' });
    }
    ...
    if (tutor.googleAccessToken) {
      try {
        ...
        console.error('Erro ao criar evento no Calendar:', calendarError.message);
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (sessao.tutor._id.toString() !== req.userId && sessao.aluno._id.toString() !== req.userId) {
...
router.put('/:id/confirmar', authMiddleware, async (req, res) => {
  try {
    ...
    if (sessao.tutor.toString() !== req.userId) {
      return res.status(403).json({ error: 'Apenas o tutor pode confirmar a sessão' });
    }
    
    if (sessao.status !== 'Pendente') {
      return res.status(400).json({ error: 'Apenas sessões pendentes podem ser confirmadas' });
    }
...
router.put('/:id/concluir', authMiddleware, async (req, res) => {
  try {
    ...
    // Verificar se o usuário faz parte da sessão
    if (sessao.tutor.toString() !== req.userId && sessao.aluno.toString() !== req.userId) {
      return res.status(403).json({ error: 'Acesso negado' });
    }
    
    if (sessao.status === 'Cancelada') {
      return res.status(400).json({ error: 'Sessão cancelada não pode ser concluída' });
    }
...
router.put('/:id/cancelar', authMiddleware, async (req, res) => {
  try {
    ...
    if (sessao.status === 'Concluída') {
      return res.status(400).json({ error: 'Sessão concluída não pode ser cancelada' });
    }
    ...
    if (sessao.googleEventId) {
      try {
        ...
        console.error('Erro ao cancelar evento no Calendar:', calendarError.message);
      }
    }
...
router.post('/avaliar', authMiddleware, async (req, res) => {
  try {
    ...
    if (sessao.aluno.toString() !== req.userId) {
      return res.status(403).json({ error: 'Apenas o aluno pode avaliar a sessão' });
    }
    
    if (sessao.status !== 'Concluída') {
      return res.status(400).json({ error: 'Apenas sessões concluídas podem ser avaliadas' });
    }
    ...
    const mediaNotas = todasAvaliacoes.reduce((acc, av) => acc + av.nota, 0) / todasAvaliacoes.length;
