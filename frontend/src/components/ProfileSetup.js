...
function ProfileSetup({ token, onComplete }) {
  ...
  const adicionarDisciplinaDomina = () => {
    ...
    if (
      formData.disciplinas_dominadas.some(
        (d) =>
          d.disciplina.toLowerCase() === novaDisciplinaDomina.toLowerCase(),
      )
    ) {
      toast.error("Já adicionada");
      return;
    }
    setFormData({
      ...formData,
  ...
  const adicionarDisciplinaPrecisa = () => {
    ...
    if (
      formData.disciplinas_precisa.some(
        (d) =>
          d.disciplina.toLowerCase() === novaDisciplinaPrecisa.toLowerCase(),
      )
    ) {
      toast.error("Já adicionada");
      return;
  ...
  const handleSave = async () => {
    ...
    if (formData.isTutor && formData.disciplinas_dominadas.length === 0) {
      toast.warning(
        "Você será listado como tutor, mas precisa adicionar disciplinas para aparecer nas buscas.",
      );
    }
  ...
  const canProceed = () => {
    ...
    if (step === 3 && formData.isTutor)
      return formData.disciplinas_dominadas.length > 0;
    return true;
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-center mb-8 gap-2">
          {Array.from({ length: getTotalSteps() }, (_, i) => i + 1).map((s) => (
            <React.Fragment key={s}>
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition-all ${step > s ? "bg-violet-600 text-white" : step === s ? "bg-violet-600 text-white ring-4 ring-violet-200" : "bg-white text-gray-400 border-2"}`}
              >
                {step > s ? <Check className="w-5 h-5" /> : s}
              </div>
              {s < getTotalSteps() && (
                <div
                  className={`w-12 h-1 ${step > s ? "bg-violet-600" : "bg-gray-200"}`}
                />
              )}
            </React.Fragment>
          ))}
        ...
        <Card className="shadow-xl border-none">
          <CardHeader>
            ...
            <CardDescription>
              {step === 1 &&
                "Escolha se quer apenas aprender ou também ensinar"}
              {step === 2 && "Adicione disciplinas que você precisa de ajuda"}
              {step === 3 &&
                formData.isTutor &&
                "Adicione disciplinas que você domina"}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            {step === 1 && (
              <div className="space-y-4">
                <div className="grid gap-4">
                  <button
                    onClick={() => setFormData({ ...formData, isTutor: false })}
                    className={`p-6 rounded-xl border-2 text-left transition-all ${!formData.isTutor ? "border-violet-600 bg-violet-50" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <div className="flex items-start gap-4">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center ${!formData.isTutor ? "bg-violet-600 text-white" : "bg-gray-100 text-gray-500"}`}
                      >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">
                          Encontre tutores para aprender
                        </p>
                      </div>
                      {!formData.isTutor && (
                  ...
                  <button
                    onClick={() => setFormData({ ...formData, isTutor: true })}
                    className={`p-6 rounded-xl border-2 text-left transition-all ${formData.isTutor ? "border-violet-600 bg-violet-50" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <div className="flex items-start gap-4">
                      <div
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">
                          Compartilhe conhecimento ajudando outros
                        </p>
                      </div>
                      {formData.isTutor && (
                        <Check className="w-6 h-6 text-violet-600" />
                ...
                <div className="pt-4 space-y-4">
                  ...
                  <div>
                    <Label className="mb-2 block">Bio (opcional)</Label>
                    <Textarea
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <Input
                  placeholder="Ou digite uma disciplina"
                  value={novaDisciplinaPrecisa}
                  onChange={(e) => setNovaDisciplinaPrecisa(e.target.value)}
                  onKeyPress={(e) =>
                    e.key === "Enter" && adicionarDisciplinaPrecisa()
                  }
                />
                <div className="flex flex-wrap gap-2 min-h-[80px] p-4 border rounded-xl bg-gray-50">
                  {formData.disciplinas_precisa.length > 0 ? (
                    formData.disciplinas_precisa.map((d, i) => (
                      <Badge
                        key={i}
                        className="bg-indigo-100 text-indigo-700 pr-1 py-1.5"
                      >
                        ...
                        <button
                          onClick={() =>
                            setFormData({
                              ...formData,
                              disciplinas_precisa:
                                formData.disciplinas_precisa.filter(
                                  (_, idx) => idx !== i,
                                ),
                            })
                          }
            ...
            {step === 3 && formData.isTutor && (
              <div className="space-y-4">
                <div className="bg-violet-50 border border-violet-200 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm text-violet-700">
                      Adicione disciplinas que você tem conhecimento para
                      ensinar
                    </p>
                  </div>
                </div>
                ...
                <Input
                  ...
                  onKeyPress={(e) =>
                    e.key === "Enter" && adicionarDisciplinaDomina()
                  }
                />
                <div className="flex flex-wrap gap-2 min-h-[80px] p-4 border rounded-xl bg-gray-50">
                  {formData.disciplinas_dominadas.length > 0 ? (
                    formData.disciplinas_dominadas.map((d, i) => (
                      <Badge
                        key={i}
                        className="bg-violet-100 text-violet-700 pr-1 py-1.5"
                      >
                        ...
                        <button
                          onClick={() =>
                            setFormData({
                              ...formData,
                              disciplinas_dominadas:
                                formData.disciplinas_dominadas.filter(
                                  (_, idx) => idx !== i,
                                ),
                            })
                          }
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
                  <strong>Dica:</strong> Após concluir, vá em "Meu Perfil" para
                  adicionar suas datas e horários disponíveis para tutorias.
                </div>
              </div>
            )}
