...
function Profile({ token, user, onUpdate }) {
  ...
  const removerDisciplinaDomina = (index) => {
    setFormData({
      ...formData,
      disciplinas_dominadas: formData.disciplinas_dominadas.filter(
        (_, i) => i !== index,
      ),
    });
  };
  ...
  const removerDisciplinaPrecisa = (index) => {
    setFormData({
      ...formData,
      disciplinas_precisa: formData.disciplinas_precisa.filter(
        (_, i) => i !== index,
      ),
    });
  };
  ...
  const adicionarDisponibilidade = async () => {
    if (
      !novaDisponibilidade.data ||
      !novaDisponibilidade.horario_inicio ||
      !novaDisponibilidade.horario_fim
    ) {
  ...
  const removerDisponibilidade = async (id) => {
    try {
      const response = await axios.delete(
        `${API_URL}/users/disponibilidade/${id}`,
        { headers },
      );
      setFormData({
        ...formData,
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Disciplinas que Preciso Aprender
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 min-h-[40px]">
            {formData.disciplinas_precisa.map((d, i) => (
              <Badge
                key={i}
                className="bg-indigo-100 text-indigo-700 pr-1 py-1"
              >
                {d.disciplina} • {d.nivel_desejado}
                <button
                  onClick={() => removerDisciplinaPrecisa(i)}
                  className="ml-2"
                >
                  <X className="w-3 h-3" />
                </button>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px]">
              {formData.disciplinas_dominadas.map((d, i) => (
                <Badge
                  key={i}
                  className="bg-violet-100 text-violet-700 pr-1 py-1"
                >
                  {d.disciplina} • {d.nivel}
                  <button
                    onClick={() => removerDisciplinaDomina(i)}
                    className="ml-2"
                  >
                    <X className="w-3 h-3" />
                  </button>
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-700">
              Adicione as datas e horários em que você está disponível para dar
              tutorias.
            </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {disponibilidadesPorData[dataKey].map((disp) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => removerDisponibilidade(disp._id)}
